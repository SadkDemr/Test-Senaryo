(venv) C:\OtomasyoTool>echo path = r"C:\OtomasyoTool\backend\venv\Lib\site-packages\pywinauto\base_wrapper.py" > fix.py echo f = open(path, "r", encoding="utf-8") >> fix.py echo content = f.read() >> fix.py echo f.close() >> fix.py echo content = content.replace("import win32ui", "try:\n import win32ui\nexcept (ImportError, OSError):\n win32ui = None", 1) >> fix.py echo f = open(path, "w", encoding="utf-8") >> fix.py echo f.write(content) >> fix.py echo f.close() >> fix.py echo print("Duzeltildi") >> fix.py python fix.py
